...
const ChartComponent = ({ selectedSymbol }) => {
  ...
  useEffect(() => {
    ...
    if (!chart) {
      const newChart = createChart(chartContainerRef.current, {
        layout: { background: { type: 'solid', color: 'transparent' }, textColor: '#808080', lineColor: '#29f1ff' },
        grid: { vertLines: { color: 'rgba(42, 46, 57, 0.5)' }, horzLines: { color: 'rgba(42, 46, 57, 0.5)' } },
        rightPriceScale: { borderVisible: false, scaleMargins: { top: 0.2, bottom: 0.2 } },
        timeScale: { borderVisible: false, timeVisible: true, secondsVisible: true, rightBarStaysOnScroll: true, fixLeftEdge: true },
        crosshair: { mode: 0, vertLine: { labelBackgroundColor: 'rgb(29, 29, 29)' }, horzLine: { labelBackgroundColor: 'rgb(29, 29, 29)' } },
        width: chartContainerRef.current.clientWidth,
        height: 510,
      });
      candlestickSeries.current = newChart.addCandlestickSeries({
        upColor: '#29f1ff', downColor: '#808080', wickUpColor: '#29f1ff', wickDownColor: '#808080', borderUpColor: '#29f1ff', borderDownColor: '#808080',
      });
      volumeSeries.current = newChart.addHistogramSeries({
        color: 'rgb(69, 69, 69)', lineWidth: 2, priceFormat: { type: 'volume' }, priceScaleId: '',
      });
  ...
  }, [chartData, interval]); // Also update on interval change
